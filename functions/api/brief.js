...
export async function onRequest(context) {
  ...
  try {
    if (data.action === 'plan') {
      ...
      return json({ plan: { title: plan.title, sections: plan.sections.map(section => ({ id: section.id, title: section.title, brief: section.brief })) } });
    }
